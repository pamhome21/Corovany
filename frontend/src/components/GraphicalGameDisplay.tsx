    ...
    return <svg height={850} width={800}>
        ...
        {(gameState.state === 'combatReady' || gameState.state === 'finished') && currentUnit && <>
            ...
            {currentUnit.Character.CharClass.Perks.map((perk, index) =>
                <svg className={'hover-target'} onClick={() => dispatch(SelectPerk(perk))} key={index} y={590} x={200 + index * 150}>
...
function UnitComponent({unit, index}: UnitProps) {
    ...
    return <svg onClick={executeNextTurnCommand} y={320 + index * 90} x={(enemy ? 500 : 220) - index * 20 * (enemy ? -1 : 1)}>
...
function UnitInQueue({unit, index}: UnitProps) {
    ...
    return <svg className={'hover-target'} x={-4 + index * 54}>
        ...
        <foreignObject y={60} x={0} width={120} height={120}>
